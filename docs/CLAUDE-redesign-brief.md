...
4. **Vocabulary is fixed** (from the glossary). Use these words in UI copy:
   ...
   - Reactions: **Love this** (heart), **Count me in** (raised hand), **Add a thought** (comment bubble) — icon-only buttons, three across. **Keep going** is retired as a button; it survives only as a quick starter inside Add a thought. ~~Counts are **maker-only**: nobody but a Moment's own maker ever sees a number, on their own Moment only — never a setting, never shown to anyone else.~~ **Superseded Sept 24, 2026**: Love this/Count me in totals are now public, shown to anyone who can see the Moment — Thoughts stays the one maker-only count. **Bookmark** is separate and private. See `docs/moment-card-and-reactions-spec.md`'s STATUS block for the full, current anatomy.
