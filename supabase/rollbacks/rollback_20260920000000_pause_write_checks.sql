...
-- pursuits exactly, including each pair's original role scoping (one of
-- each pair was `{public}`, the other `{authenticated}` — not identical
-- duplicates in every respect, just functionally overlapping).

drop function if exists public.write_blocked_by_pause();
