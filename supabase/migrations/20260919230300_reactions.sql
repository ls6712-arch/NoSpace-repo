...
-- table, plus a real bookmarks table. Draft only.
--
...
--
-- type values match PostReactions.tsx's own ReactionId ids exactly
-- ('love' | 'in' | 'keepgoing') rather than the more spelled-out values an
-- earlier draft used — the component is the source of truth here, not the
-- other way around.
create table if not exists public.reactions (
  id bigint generated always as identity primary key,
  post_id bigint not null references public.posts(id) on delete cascade,
  user_id uuid not null references auth.users(id) on delete cascade,
  type text not null check (type in ('love', 'in', 'keepgoing')),
...
create policy "the reactor and the post's author can see a reaction"
  ...
  using (
    ...
    or (select auth.uid()) = (select p.user_id from public.posts p where p.id = reactions.post_id)
  );

-- "Try This" (toggleSaved/isSaved in lib/journal.ts) is also localStorage-
-- only today. Real table now, owner-only for every operation — nobody
-- else, not even the post's author, needs to see what someone bookmarked.
